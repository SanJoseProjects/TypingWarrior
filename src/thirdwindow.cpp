...
ThirdWindow::ThirdWindow(QWidget* parent, QString login)
...
void ThirdWindow::on_pushButton_check_clicked()
{
    ...
    mis = text_verification(
            thirdwindow->textEdit->toPlainText(),
    ...
    if (mis == 0) {
        write_score(username, level, speed);
    } else {
        write_score(username, level, 0);
    }
...
QString ThirdWindow::on_pushButton_start_clicked()
{
    ti = 0;
    t.setHMS(0, 0, 0);
...
void ThirdWindow::write_score(QString username, QString level, int speed)
{
    QSqlDatabase db;
    ...
    QSqlQuery query;
    query.exec(
            "SELECT " + level + " FROM Board WHERE Username = '" + username
            + "'");
    if (query.next()) {
        int cur_score = query.value(0).toInt();
        if (speed > cur_score) {
            query.exec(
                    "UPDATE Board SET " + level + " = " + QString::number(speed)
                    + " WHERE Username = '" + username + "'");
        }
    } else {
        query.prepare("INSERT INTO Board (Username, " + level + ")"
                          "VALUES (:Username, :" + level + ")");
        query.bindValue(":Username", username);
        query.bindValue(":" + level, speed);
        query.exec();
    }
